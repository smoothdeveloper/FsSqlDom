module FsSqlDomGallery.GraphTableRelationships
...
type AnalysisContext(connstr:string, logger: Action<string>) =
  ...
  let fetchColumnsByTable : Lazy<Dictionary<string, HashSet<string>>> =
    lazy  
  ...
  member x.module_defs = fetchDefs.Force
  member x.tables = fetchTables.Force
  member x.columnsByTable = fetchColumnsByTable.Force
...
type JoinAnalyzer(ctx:AnalysisContext) =
  ...
  member x.AddMapping(tname:string, alias: string option) =
    ...
    ctx.log (sprintf "Adding %s..." tname)
    match alias with
    | Some(alias) -> 
  ...
  member x.ResolveTableForCol (col:string) =
    let tables = 
      [ for t in x.tables do
          if ctx.columnsByTable().[t].Contains(col) then yield t ]
    match tables with
    | [t] -> Some(t)
    | xs -> None
  ...
  member x.ResolveTable (t:string list) =
    ...
    | xs ->
      ctx.log (sprintf "Can't resolve table for %A" xs)
      None
...
let visualize (rels:Set<string*string*string>) =
  let script =
    ...
    let mutable schema_colors = initial_colors |> List.sortBy(fun _ -> rng.NextDouble())

    let color_assigns = Dictionary<string list, string>()

    let get_color (table:string) =
      match table.Split('.') |> List.ofArray |> List.rev with
      | [] | [_] -> "#97C2FC"
      | _::schema ->
          match color_assigns.TryGetValue(schema) with
          | true, color -> color
          | _ ->
            match schema_colors with
            | color::colors ->
              schema_colors <- colors
              color_assigns.[schema] <- color
              color
            | _ -> "#97C2FC"
...
let analyzeAndVisualize(connstr:string, logger: Action<string>) =
  let ctx = AnalysisContext(connstr, logger)
  let realTables = ctx.tables()
  let relationships = HashSet<_>(HashIdentity.Structural)

  for def in ctx.module_defs() do
    ctx.log (sprintf "Processing %20s %s" def.type_desc def.name)
    try
      ...
      | Choice1Of2 sql ->
        ...
        | None -> ()
      | Choice2Of2 errs -> 
        ctx.log (sprintf "Could not parse %s" def.name)
        for err in errs do printfn "\t%s" (err.ToString())

    with ex -> ctx.log (sprintf "Failed on %s:\n\n\t%s\n" def.name ex.Message)
  
  ctx.log (sprintf "Result: %A" realTables)
  let relationships = 
    relationships 
    |> Seq.filter (fun (a, b, _) -> (realTables.Contains a) && (realTables.Contains b))
    |> Set.ofSeq
  ctx.log (sprintf "Result: %A" relationships)
